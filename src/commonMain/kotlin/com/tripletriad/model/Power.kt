...
/**
 * All four of [effectivePower], for a caller that wants to *draw* a card rather than fight with it.
 *
 * A card's own [Card.top]..[Card.left] are what is printed on it, and under any of these rules that
 * is no longer what it is worth. A board that shows the printed four asks every player to hold a
 * running per-type count in their head and apply it, per side, with the clamp — which is the
 * arithmetic the screen has the data to do.
 *
 * Returned as a value rather than written back onto a [Card] because a [Card] may not hold these
 * numbers: its `init` requires 1..10 and an effective power can be 0.
 */
fun effectivePowers(
    card: Card,
    rules: GameRules,
    ...
    top = effectivePower(card, Side.TOP, rules, element, tally),
    right = effectivePower(card, Side.RIGHT, rules, element, tally),
    bottom = effectivePower(card, Side.BOTTOM, rules, element, tally),
    left = effectivePower(card, Side.LEFT, rules, element, tally),
)

/**
 * Four edge powers, in the game's own order — top, right, bottom, left.
 *
 * The order is `power[0..3]` as `CardDigits.display()` states it: the order the data is in, the
 * order the card face draws, and the order the rest of this port reads.
 */
data class EdgePowers(val top: Int, val right: Int, val bottom: Int, val left: Int) {

    operator fun get(side: Side): Int = when (side) {
        Side.TOP -> top
        Side.RIGHT -> right
        Side.BOTTOM -> bottom
        Side.LEFT -> left
    ...
    companion object {
        /** What is printed on [card], before any rule touches it. */
        fun printed(card: Card) = EdgePowers(card.top, card.right, card.bottom, card.left)
    }
}
